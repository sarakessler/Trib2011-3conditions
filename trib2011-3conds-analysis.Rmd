...
Experiment 3 (condition 3) - The images are in real pictures in random order for each display rather than either ascending or descending, of objects that are smaller in real life than in the images of them.
...
The other type of analysis is binary logistic regression. This uses a data frame in which each row represents a check box (so that each trial spans seven rows). The analysis looks at the likelihood of each checkbox being checked and asks at which box the proabblility switches to be more likely to be checked (that can be thought of as the threshold of the adjective, or the determiner of the size of the zone). 
...
#                   trial_num = j,
#                    pic_size = jd$answers$data$sizes[j][[1]],
#                    difference_from_prev = jd$answers$data$sizes[j-1][[1]]-jd$answers$data$sizes[j-][[1]],
#                    difference_from_next = jd$answers$data$sizes[j-+][[1]]-jd$answers$data$sizes[j-][[1]],
...
#
```
Note: Manual exclusion of anon-34 due to some error in elapsed_ms where there are 27 numbers instead of 26 - must be due to some weird unidentified bug.
...
table(as.factor(d.raw$condition), d.raw$none_checked)
table(as.factor(d.raw$condition), d.raw$all_checked)

all.none.raw <- d.raw %>%
  filter(adj != "pretty") %>%
  filter(adj != "ugly")%>%
  group_by(adj,condition)%>%
  summarise(all.c = sum(all_checked), none.c = sum(none_checked), seven.c = sum(!all_checked&(num_checked==7)))
      
sum(d.raw$all_checked)
...
for (i in 1:length(d.allconditions$none_checked)) {
   if (d.allconditions$none_checked[i] == TRUE & (!is.na(d.allconditions$none_checked[i]))){
   d.allconditions$num_checked[i] = 0
   }
 }



all.none <- d.allconditions %>%
  filter(adj != "pretty") %>%
  filter(adj != "ugly")%>%
  group_by(adj,condition)%>%
  summarise(all.c = sum(all_checked), none.c = sum(none_checked), seven.c = sum(!all_checked&(num_checked==7)))
...
table(as.factor(d$gender))/48
table(as.factor(d$workerid))
#for pilot A
...
write.table(d_wilc, "D:/Dropbox/School/more adjs/experiment 1b/Trib2011-3conditions/data/d_wilc.txt", sep="\t")
...
ggplot(d_graph, aes(x=adj, y=mean_zone, fill = prototype_status)) + geom_bar(stat = "identity", position=position_dodge()) +
    geom_errorbar(aes(ymin=mean_zone-cis, ymax=mean_zone+cis),
                  width=.2, position=position_dodge(.9)) + ggtitle("Experiment 1c-con1 - real pics ordered - Mean zone by prototype status")

#graph for cogsci paper
theme_set(theme_bw(base_size = 18))
png(filename = "fig-exp2-res.png", width = 601, height = 373)
...
theme_set(theme_bw(base_size = 18))
png(filename = "semprag-fig-exp2-res.png", width = 601, height = 373)
print(ggplot(d_graph, aes(x=adj, y=mean_zone, fill = prototype_status)) + 
        geom_bar(stat = "identity", position=position_dodge()) +
    geom_errorbar(aes(ymin=mean_zone-cis, ymax=mean_zone+cis), width=.2, position=position_dodge(.9)) + 
      ggtitle("Mean zone by prototypicality status") +  
...
d_var <- d %>%
  group_by(subid) %>%
  summarize(num_answers = length(levels(as.factor(num_checked))), mean_num_resps = mean(num_checked), sd_num_resps = sd(num_checked))

ggplot(d_var, aes(x=mean_num_resps)) + geom_histogram(binwidth=.2)
ggplot(d_var, aes(x=num_answers)) + geom_histogram(binwidth=1)

d <- d %>%
  left_join(d_var)

png(filename = "fig-variation-by-subj.png", width = 700, height = 800)

print(ggplot(data=d, aes(x=quest_order, y = num_checked))  +
    geom_line()  + facet_wrap( ~ subid, ncol=5) + xlab("Trial number") + ylab("Number of images selected") )
dev.off()

#variation by subject ordered photos, trials 1-24
png(filename = "fig-variation-by-subj-first24.png", width = 700, height = 800)

print(ggplot(data=subset(d,quest_order<27), aes(x=quest_order, y = num_checked))  +
    geom_line()  + facet_wrap( ~ subid, ncol=5) + xlab("Trial number") + ylab("Number of images selected") )
dev.off()


d_var2 <- d.allconditions %>%
  filter(condition == "2")%>%
  group_by(subid) %>%
  summarize(num_answers = length(levels(as.factor(num_checked))), mean_num_resps = mean(num_checked), sd_num_resps = sd(num_checked))

ggplot(d_var2, aes(x=mean_num_resps)) + geom_histogram(binwidth=.2)
ggplot(d_var2, aes(x=num_answers)) + geom_histogram(binwidth=1)
...
d2$subnum <- as.numeric(gsub(".json", "", gsub("anon-", "", d2$subid)))

png(filename = "fig-variation-by-subj-exp3-1.png", width = 700, height = 800)
print(
ggplot(data=subset(d2,subnum<150), aes(x=quest_order, y = num_checked))  +
    geom_line()  + facet_wrap( ~ subid, ncol=5)+ xlab("Trial number") + ylab("Number of images selected")) 
dev.off()

png(filename = "fig-variation-by-subj-exp3-2.png", width = 700, height = 800)
print(ggplot(data=subset(d2,subnum>150), aes(x=quest_order, y = num_checked))  +
    geom_line()  + facet_wrap( ~ subid, ncol=5) )
dev.off()

d_var3 <- d.allconditions %>%
  filter(condition == "3")%>%
  group_by(subid) %>%
  summarize(num_answers = length(levels(as.factor(num_checked))), mean_num_resps = mean(num_checked), sd_num_resps = sd(num_checked))

ggplot(d_var3, aes(x=mean_num_resps)) + geom_histogram(binwidth=.2)
ggplot(d_var3, aes(x=num_answers)) + geom_histogram(binwidth=1)
...
d3$subnum <- as.numeric(gsub(".json", "", gsub("anon-", "", d3$subid)))

png(filename = "fig-variation-by-subj-exp5-1.png", width = 700, height = 800)
print(
ggplot(data=subset(d3,subnum<=95), aes(x=quest_order, y = num_checked))  +
    geom_line()  + facet_wrap( ~ subid, ncol=5)+ xlab("Trial number") + ylab("Number of images selected")) 
dev.off()

png(filename = "fig-variation-by-subj-exp5-2.png", width = 700, height = 800)
print(ggplot(data=subset(d3,subnum>95), aes(x=quest_order, y = num_checked))  +
    geom_line()  + facet_wrap( ~ subid, ncol=5) )
dev.off()
```
...
```{r}
#graph without ducklings and bunnies
d_graph_ndb <- d %>%
  filter(noun != "bunny") %>%
  filter(noun != "duckling") %>%
  group_by(prototype_status, adj) %>%
  summarise(mean_zone = mean(num_checked), sems = sem(num_checked), cis = ci95(num_checked))

ggplot(d_graph_ndb, aes(x=adj, y=mean_zone, fill = prototype_status)) + geom_bar(stat = "identity", position=position_dodge()) +
    geom_errorbar(aes(ymin=mean_zone-cis, ymax=mean_zone+cis),
                  width=.2, position=position_dodge(.9))
```
...
```{r}
by_obj_adj1 <- d.allconditions %>%
  filter(condition == "1")%>%
  group_by(noun, prototype_status,adj)%>%
  summarise(mean_zone = mean(num_checked), sems = sem(num_checked), cis = ci95(num_checked))
#graph of mean number checked by object for both adjectives, condition 1 - ordered photos
#raph for Meghan meeting 19.7.18
png(filename = "fig-exp2-by-item.png", width = 700, height = 373)
print(ggplot(by_obj_adj1, aes(x=reorder(noun, mean_zone), y=mean_zone, fill=prototype_status)) + geom_bar(stat = "identity", position=position_dodge()) + facet_wrap(~adj) +
    geom_errorbar(aes(ymin=mean_zone-cis, ymax=mean_zone+cis),
                  width=.2, position=position_dodge(.9)) + 
      theme (plot.title = element_text(lineheight=.8, face="bold", hjust=0.5), axis.text.x  = element_text(angle=90, vjust=0.5)) + 
  xlab("noun") + ggtitle("Mean zone by object for each\nadjective - ordered photos") )
dev.off()
```
```{r}
by_obj_adj1s <- d.allconditions %>%
  filter(condition == "1")%>%
  filter(adj == "small")%>%
  group_by(noun, prototype_status,adj)%>%
  summarise(mean_zone = mean(num_checked), sems = sem(num_checked), cis = ci95(num_checked))

by_obj_adj1b <- d.allconditions %>%
  filter(condition == "1")%>%
  filter(adj == "big")%>%
  group_by(noun, prototype_status,adj)%>%
  summarise(mean_zone = mean(num_checked), sems = sem(num_checked), cis = ci95(num_checked))
#graph of mean number checked by object when adj small, condition 1 - ordered photos
#graph for Meghan meeting 19.7.18
g1 <- ggplot(by_obj_adj1s, aes(x=reorder(noun, mean_zone), y=mean_zone, fill=prototype_status)) + geom_bar(stat = "identity", position=position_dodge()) + 
    geom_errorbar(aes(ymin=mean_zone-cis, ymax=mean_zone+cis),
                  width=.2, position=position_dodge(.9)) + 
      theme (plot.title = element_text(lineheight=.8, face="bold", hjust=0.5), axis.text.x  = element_text(angle=90, vjust=0.5)) + 
  xlab("noun") + ggtitle("Mean zone by object when\nadjective is small - ordered photos") +
      scale_fill_brewer(name="Prot.\nstatus", breaks=c("big", "neither", "small"),
                         labels=c("Big", "Neutral", "Small"), palette="YlGnBu") + 
  scale_y_continuous(limits=c(0, 3.7))
g2 <- ggplot(by_obj_adj1b, aes(x=reorder(noun, mean_zone), y=mean_zone, fill=prototype_status)) + geom_bar(stat = "identity", position=position_dodge()) + 
    geom_errorbar(aes(ymin=mean_zone-cis, ymax=mean_zone+cis),
                  width=.2, position=position_dodge(.9)) + 
      theme (plot.title = element_text(lineheight=.8, face="bold", hjust=0.5), axis.text.x  = element_text(angle=90, vjust=0.5), legend.position="none") + 
  xlab("noun") + ggtitle("Mean zone by object when\nadjective is big - ordered photos")+
      scale_fill_brewer(name="Prot.\nstatus", breaks=c("big", "neither", "small"),
                         labels=c("Big", "Neutral", "Small"), palette="YlGnBu") + 
  scale_y_continuous(limits=c(0, 3.7))
png(filename = "fig-exp2-by-item.png", width = 900, height = 500)
print(multiplot(g1, g2, cols=2))
dev.off()

#with no legend for dissertation
g5 <- ggplot(by_obj_adj1s, aes(x=reorder(noun, mean_zone), y=mean_zone, fill=prototype_status)) + geom_bar(stat = "identity", position=position_dodge()) + 
    geom_errorbar(aes(ymin=mean_zone-cis, ymax=mean_zone+cis),
                  width=.2, position=position_dodge(.9)) + 
      theme (plot.title = element_text(lineheight=.8, face="bold", hjust=0.5), axis.text.x  = element_text(angle=90, vjust=0.5), legend.position="none") + 
  xlab("noun") + ggtitle("Mean zone by object when\nadjective is small - ordered photos") +
      scale_fill_brewer(name="Prot.\nstatus", breaks=c("big", "neither", "small"),
                         labels=c("Big", "Neutral", "Small"), palette="YlGnBu") + 
  scale_y_continuous(limits=c(0, 3.7))
g6 <- ggplot(by_obj_adj1b, aes(x=reorder(noun, mean_zone), y=mean_zone, fill=prototype_status)) + geom_bar(stat = "identity", position=position_dodge()) + 
    geom_errorbar(aes(ymin=mean_zone-cis, ymax=mean_zone+cis),
                  width=.2, position=position_dodge(.9)) + 
      theme (plot.title = element_text(lineheight=.8, face="bold", hjust=0.5), axis.text.x  = element_text(angle=90, vjust=0.5), legend.position="none") + 
  xlab("noun") + ggtitle("Mean zone by object when\nadjective is big - ordered photos")+
      scale_fill_brewer(name="Prot.\nstatus", breaks=c("big", "neither", "small"),
                         labels=c("Big", "Neutral", "Small"), palette="YlGnBu") + 
  scale_y_continuous(limits=c(0, 3.7))
png(filename = "fig-exp2-by-item-no-legend.png", width = 900, height = 500)
print(multiplot(g6, g5, cols=2))
dev.off()
```
...
#graph of mean number checked by object when adj small, condition 1 - ordered photos
#graph for Meghan meeting 19.7.18
png(filename = "fig-exp2-by-item-small.png", width = 700, height = 373)
print(ggplot(by_obj_adj1s, aes(x=reorder(noun, mean_zone), y=mean_zone, fill=prototype_status)) + geom_bar(stat = "identity", position=position_dodge()) + 
    geom_errorbar(aes(ymin=mean_zone-cis, ymax=mean_zone+cis),
                  width=.2, position=position_dodge(.9)) + 
      theme (plot.title = element_text(lineheight=.8, face="bold", hjust=0.5), axis.text.x  = element_text(angle=90, vjust=0.5)) + 
  xlab("noun") + ggtitle("Mean zone by object for each\nadjective - ordered photos") )
...
```{r}
by_obj_adj2s <- d.allconditions %>%
  filter(condition == "2")%>%
  filter(adj == "small")%>%
  group_by(noun, prototype_status,adj)%>%
  summarise(mean_zone = mean(num_checked), sems = sem(num_checked), cis = ci95(num_checked))

by_obj_adj2b <- d.allconditions %>%
  filter(condition == "2")%>%
  filter(adj == "big")%>%
  group_by(noun, prototype_status,adj)%>%
  summarise(mean_zone = mean(num_checked), sems = sem(num_checked), cis = ci95(num_checked))
#graph of mean number checked by object when adj small, condition 2 - randomized photos
#graph for Meghan meeting 19.7.18
g3 <- ggplot(by_obj_adj2s, aes(x=reorder(noun, mean_zone), y=mean_zone, fill=prototype_status)) + geom_bar(stat = "identity", position=position_dodge()) + 
    geom_errorbar(aes(ymin=mean_zone-cis, ymax=mean_zone+cis),
                  width=.2, position=position_dodge(.9)) + 
      theme (plot.title = element_text(lineheight=.8, face="bold", hjust=0.5), axis.text.x  = element_text(angle=90, vjust=0.5)) + 
  xlab("noun") + ggtitle("Mean zone by object when\nadjective is small - randomized photos") +
      scale_fill_brewer(name="Prot.\nstatus", breaks=c("big", "neither", "small"),
                         labels=c("Big", "Neutral", "Small"), palette="YlGnBu") + 
  scale_y_continuous(limits=c(0, 3.5))
g4 <- ggplot(by_obj_adj2b, aes(x=reorder(noun, mean_zone), y=mean_zone, fill=prototype_status)) + geom_bar(stat = "identity", position=position_dodge()) + 
    geom_errorbar(aes(ymin=mean_zone-cis, ymax=mean_zone+cis),
                  width=.2, position=position_dodge(.9)) + 
      theme (plot.title = element_text(lineheight=.8, face="bold", hjust=0.5), axis.text.x  = element_text(angle=90, vjust=0.5), legend.position="none") + 
  xlab("noun") + ggtitle("Mean zone by object when\nadjective is big - randomized photos") +
      scale_fill_brewer(name="Prot.\nstatus", breaks=c("big", "neither", "small"),
                         labels=c("Big", "Neutral", "Small"), palette="YlGnBu") + 
  scale_y_continuous(limits=c(0, 3.5))
png(filename = "fig-exp3-by-item.png", width = 900, height = 500)
print(multiplot(g3, g4, cols=2))
dev.off()

#figure with no legend for dissertation
g7 <- ggplot(by_obj_adj2s, aes(x=reorder(noun, mean_zone), y=mean_zone, fill=prototype_status)) + geom_bar(stat = "identity", position=position_dodge()) + 
    geom_errorbar(aes(ymin=mean_zone-cis, ymax=mean_zone+cis),
                  width=.2, position=position_dodge(.9)) + 
      theme (plot.title = element_text(lineheight=.8, face="bold", hjust=0.5), axis.text.x  = element_text(angle=90, vjust=0.5), legend.position="none") + 
  xlab("noun") + ggtitle("Mean zone by object when\nadjective is small - randomized photos") +
      scale_fill_brewer(name="Prot.\nstatus", breaks=c("big", "neither", "small"),
                         labels=c("Big", "Neutral", "Small"), palette="YlGnBu") + 
  scale_y_continuous(limits=c(0, 3.5))
g8 <- ggplot(by_obj_adj2b, aes(x=reorder(noun, mean_zone), y=mean_zone, fill=prototype_status)) + geom_bar(stat = "identity", position=position_dodge()) + 
    geom_errorbar(aes(ymin=mean_zone-cis, ymax=mean_zone+cis),
                  width=.2, position=position_dodge(.9)) + 
      theme (plot.title = element_text(lineheight=.8, face="bold", hjust=0.5), axis.text.x  = element_text(angle=90, vjust=0.5), legend.position="none") + 
  xlab("noun") + ggtitle("Mean zone by object when\nadjective is big - randomized photos") +
      scale_fill_brewer(name="Prot.\nstatus", breaks=c("big", "neither", "small"),
                         labels=c("Big", "Neutral", "Small"), palette="YlGnBu") + 
  scale_y_continuous(limits=c(0, 3.5))
png(filename = "fig-exp3-by-item-no-legend.png", width = 900, height = 500)
print(multiplot(g8, g7, cols=2))
dev.off()
```

```{r}
by_obj_adj2 <- d.allconditions %>%
  filter(condition == "2")%>%
  group_by(noun, prototype_status,adj)%>%
  summarise(mean_zone = mean(num_checked), sems = sem(num_checked), cis = ci95(num_checked))
#graph of mean number checked by object for both adjectives, condition 2 - random photographs
#raph for Meghan meeting 19.7.18
png(filename = "fig-exp3-by-item.png", width = 700, height = 373)
print(ggplot(by_obj_adj2, aes(x=reorder(noun, mean_zone), y=mean_zone, fill=prototype_status)) + geom_bar(stat = "identity", position=position_dodge()) + facet_wrap(~adj) +
    geom_errorbar(aes(ymin=mean_zone-cis, ymax=mean_zone+cis),
                  width=.2, position=position_dodge(.9)) + 
      theme (plot.title = element_text(lineheight=.8, face="bold", hjust=0.5), axis.text.x  = element_text(angle=90, vjust=0.5)) + 
  xlab("noun") + ggtitle("Mean zone by object for each\nadjective - randomized photos") )
dev.off()
```

```{r}
by_obj_adj3 <- d.allconditions %>%
  filter(condition == "3")%>%
  group_by(noun, prototype_status,adj)%>%
  summarise(mean_zone = mean(num_checked), sems = sem(num_checked), cis = ci95(num_checked))
#graph of mean number checked by object for both adjectives, condition 3- random tiny objects
#raph for Meghan meeting 19.7.18
png(filename = "fig-exp5-by-item.png", width = 700, height = 373)
print(ggplot(by_obj_adj3, aes(x=reorder(noun, mean_zone), y=mean_zone, fill=prototype_status)) + geom_bar(stat = "identity", position=position_dodge()) + facet_wrap(~adj) +
    geom_errorbar(aes(ymin=mean_zone-cis, ymax=mean_zone+cis),
                  width=.2, position=position_dodge(.9)) + 
      theme (plot.title = element_text(lineheight=.8, face="bold", hjust=0.5), axis.text.x  = element_text(angle=90, vjust=0.5)) + 
  xlab("noun") + ggtitle("Mean zone by object for each\nadjective - replication") )
dev.off()
```

```{r}
by_obj_adj3s <- d.allconditions %>%
  filter(condition == "3")%>%
  filter(adj == "small")%>%
  group_by(noun, prototype_status,adj)%>%
  summarise(mean_zone = mean(num_checked), sems = sem(num_checked), cis = ci95(num_checked))

by_obj_adj3b <- d.allconditions %>%
  filter(condition == "3")%>%
  filter(adj == "big")%>%
  group_by(noun, prototype_status,adj)%>%
  summarise(mean_zone = mean(num_checked), sems = sem(num_checked), cis = ci95(num_checked))
#graph of mean number checked by object when adj small, condition 3 - randomized tiny things
#graph for Meghan meeting 19.7.18
g5 <- ggplot(by_obj_adj3s, aes(x=reorder(noun, mean_zone), y=mean_zone)) + geom_bar(stat = "identity", position=position_dodge()) + 
    geom_errorbar(aes(ymin=mean_zone-cis, ymax=mean_zone+cis),
                  width=.2, position=position_dodge(.9)) + 
      theme (plot.title = element_text(lineheight=.8, face="bold", hjust=0.5), axis.text.x  = element_text(angle=90, vjust=0.5)) + 
  xlab("noun") + ggtitle("Mean zone by object when\nadjective is small - randomized tiny") 
g6 <- ggplot(by_obj_adj3b, aes(x=reorder(noun, mean_zone), y=mean_zone)) + geom_bar(stat = "identity", position=position_dodge()) + 
    geom_errorbar(aes(ymin=mean_zone-cis, ymax=mean_zone+cis),
                  width=.2, position=position_dodge(.9)) + 
      theme (plot.title = element_text(lineheight=.8, face="bold", hjust=0.5), axis.text.x  = element_text(angle=90, vjust=0.5), legend.position="none") + 
  xlab("noun") + ggtitle("Mean zone by object when\nadjective is big - randomized tiny")
png(filename = "fig-exp5-by-item.png", width = 800, height = 373)
print(multiplot(g5, g6, cols=2))
dev.off()
```
...
```
the *big* zone is significantly bigger for items which are neither prototypically big nor prototypically small than it is for items which are prototypically big.
...
write.table(d_wilc_bs, "D:/Dropbox/School/more adjs/experiment 1b/Trib2011-3conditions/data/d_wilc_bs.txt", sep="\t")
...
However, there is no longer a significant difference between the *big* zone for prototypically small and prototypically neutral items, or for prototypically big and prototypically small items. There is also no longer a significant difference between the *small* zone for prototypically neutral and protoypically big items.
...
Keeping the exclusion criteria the same excludes 7 subjects, which really isn't that bad, though the exclusion criteria may make less sense now that the order is mixed. The following analysis keeps the old exclusion criteria, but subsequent analyses may change this.
...
d_wilc <- d %>%
  ...
  write.table(d_wilc, "D:/Dropbox/School/more adjs/experiment 1b/Trib2011-3conditions/data/d_wilc_c2.txt", sep="\t")
```
...
d <- d %>%
  left_join(d_var)
d$subnum <- as.numeric(gsub(".json", "", gsub("anon-", "", d$subid)))

ggplot(data=subset(d,subnum<150), aes(x=quest_order, y = num_checked))  +
    geom_line()  + facet_wrap( ~ subid, ncol=5)
ggplot(data=subset(d,subnum>150), aes(x=quest_order, y = num_checked))  +
    geom_line()  + facet_wrap( ~ subid, ncol=5)
```
...
ggplot(d_graph, aes(x=adj, y=mean_zone, fill = prototype_status)) + geom_bar(stat = "identity", position=position_dodge()) +
    geom_errorbar(aes(ymin=mean_zone-cis, ymax=mean_zone+cis),
                  width=.2, position=position_dodge(.9)) + ggtitle("Experiment 1c-con2 - real pics random order - Mean zone by prototype status")


#graph for semprag talk
theme_set(theme_bw(base_size = 18))
png(filename = "semprag-fig-exp2a-res.png", width = 601, height = 373)
print(ggplot(d_graph, aes(x=adj, y=mean_zone, fill = prototype_status)) + 
        geom_bar(stat = "identity", position=position_dodge()) +
    geom_errorbar(aes(ymin=mean_zone-cis, ymax=mean_zone+cis), width=.2, position=position_dodge(.9)) + 
      ggtitle("Mean zone by prototypicality status - mixed order") +  
      xlab("Adjecitve in the question") +
      scale_fill_discrete(name="Prot.\nstatus", breaks=c("big", "neither", "small"),
                         labels=c("Big", "Neutral", "Small"))+ 
     ylab("Mean num. of images selected") + 
...
#graph for dissertation paper
theme_set(theme_bw(base_size = 18))
png(filename = "fig-exp3-res.png", width = 601, height = 373)
print(ggplot(d_graph, aes(x=adj, y=mean_zone, fill = prototype_status)) + 
        geom_bar(stat = "identity", position=position_dodge()) +
    geom_errorbar(aes(ymin=mean_zone-cis, ymax=mean_zone+cis), width=.2, position=position_dodge(.9)) + 
      ggtitle("Mean zone by prototypicality status") +  
...
wilcoxsign_test(d_wilc_bn$big ~ d_wilc_bn$neither, distribution="exact", alternative = "less")
```
...
#### Prepare data for analysis - create columns etc.
```
Again, despite the need to revisit the exclusion criteria (they are currently too harsh due to the change to mixed order), only 14 subjects are totally excluded according to the old criteria.
...
ggplot(by_obj_adjall, aes(x=reorder(noun, mean_zone), y=mean_zone, fill=adj)) + geom_bar(stat = "identity", position=position_dodge()) +
    geom_errorbar(aes(ymin=mean_zone-cis, ymax=mean_zone+cis),
                  width=.2, position=position_dodge(.9)) + 
  xlab("Noun") +
  ylab("Mean number checked")
...
ggplot(by_obj_adjb, aes(x=reorder(noun, mean_zone), y=mean_zone)) + geom_bar(stat = "identity", position=position_dodge()) +
    geom_errorbar(aes(ymin=mean_zone-cis, ymax=mean_zone+cis),
                  width=.2, position=position_dodge(.9))

#graph for dissertation chapter - mean zone by item
theme_set(theme_bw(base_size = 18))
png(filename = "fig-exp5-item-res.png", width = 755, height = 373)
print(ggplot(by_obj_adjall, aes(x=reorder(noun, mean_zone), y=mean_zone, fill=adj)) + 
        geom_bar(stat = "identity", position=position_dodge()) +
    geom_errorbar(aes(ymin=mean_zone-cis, ymax=mean_zone+cis),
                  width=.2, position=position_dodge(.9)) + 
  ggtitle("Mean zone by item") +
  scale_fill_discrete(name="Adjective", labels=c("Big", "Small"))+
  xlab("Noun") +
  ylab("Mean zone") + 
      theme(plot.title = element_text(lineheight=.8, face="bold", hjust=0.5)) )
...
ggplot(by_obj_adjs, aes(x=reorder(noun, mean_zone), y=mean_zone)) + geom_bar(stat = "identity", position=position_dodge()) +
    geom_errorbar(aes(ymin=mean_zone-cis, ymax=mean_zone+cis),
                  width=.2, position=position_dodge(.9))
...
ggplot(data=d, aes(x=quest_order, y = num_checked))  +
    geom_line()  + facet_wrap( ~ subid, ncol=5)
```
